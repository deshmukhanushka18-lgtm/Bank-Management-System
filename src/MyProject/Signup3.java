...
public class Signup3 extends JFrame implements ActionListener {
    ...
    Signup3(String formno) {
        ...
        image.setBounds(150, 25, 100, 100);
        add(image);
        ...
        JCheckBox[] boxes = {c1, c2, c3, c4, c5, c6};
        int y = 500;
        for (int i = 0; i < boxes.length; i++) {
            boxes[i].setBackground(new Color(255, 255, 255));
            boxes[i].setFont(new Font("Raleway", Font.BOLD, 16));
            boxes[i].setBounds((i % 2 == 0) ? 100 : 350, y, 200, 30);
            add(boxes[i]);
            if (i % 2 == 1) y += 50;
        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (e.getSource() == s) {
                ...
                if (fac.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Please select at least one Service Required.");
                    return;
                }

                if (!c7.isSelected()) {
                    JOptionPane.showMessageDialog(null, "Please confirm that the entered details are correct.");
                    return;
                }
